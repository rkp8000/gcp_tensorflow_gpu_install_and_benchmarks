...
It will guide you through the minimal steps to spin up a GPU-enabled VM in the Google cloud, install the latest version of TensorFlow (1.6.0 as of 3/5/2018), and get it to recognize the GPU and rapidly train a simple network.
...
To use a GPU, however, you'll need a paid account, so go to the billing page and upgrade your account. Note that you won't get charged until after you've spent your $300, so everything's still free.
...
With the box checked, click "EDIT QUOTAS", enter your name, email, and phone, and then type "1" for the requested limit, and describe why you are requesting a GPU (e.g. "training deep nets for computer vision research").
...
For Zone make sure to select a GPU-enabled zone corresponding to the zone you used for your GPU quota. E.g. if you used "us-west1", choose the zone "us-west1-b", which is the only "us-west1" zone with GPUs. To see a full list of GPU-enabled zones go [here](https://cloud.google.com/compute/docs/regions-zones/). If you select the wrong zone you won't be able to use any GPUs.
...
If you want to use run a Jupyter notebook server on your instance in the future check the boxes next to "Allow HTTP traffic" and "Allow HTTPS traffic" under "Firewall". 
...
Name the address whatever you want. I chose "tf-demo".
...
Go to the NVIDA site [https://developer.nvidia.com/cuda-downloads](https://developer.nvidia.com/cuda-downloads), click on "Legacy Releases", and choose "CUDA Toolkit 9.0 [Sept 2017]".
...
Note: the line starting with curl is all one line (`$ curl -O https://link/I/copied/to/the/clipboard/blah.blah.deb`).
...
You should see the Tesla K80 GPU that we attached to our VM instance. If you don't see it, make sure you actually increased your quota, that you didn't mix up the zones, and that you indeed added a Tesla K80 GPU when configuring your instance.

# Install cuDNN v7.0.4 for CUDA 9.0
This is a bit tricky, since we have to download this library onto our local machine then transfer it to our VM.
...
This is the CUDA library for deep neural networks (cuDNN). After you have your NVIDIA developer login info, go to the downloads page [here](https://developer.nvidia.com/rdp/cudnn-download) and select "cuDNN v7.0.4 (Nov 13, 2017), for CUDA 9.0", and click on "cuDNN v7.0.4 Library for Linux" to download it locally. If you choose the wrong version, the installation won't work.

## Download Google Cloud Platform SDK (software development kit)
Google's CLOUD SDK command-line tools provide a simple way of transferring files between local machines and your VM instances.

Download the CLOUD SDK [https://cloud.google.com/sdk/](https://cloud.google.com/sdk/) *onto your local machine* and unzip it.

To install it, on your local machine, cd into the unzipped directory and run:

`./install.sh`

which sets up your login/connection info so it knows to look at instance in your account, and adds the location of the unzipped directory to your `PATH`.
...
where you replace `<username>` and `<instance-name>` with the appropriate values for your VM. E.g., if your username is `arthurdent` and your instance name was `arthurs-tf-demo` you would run:

`gcloud compute scp cudnn-9.0-linux-x64-v7.tgz arthurdent@arthurs-tf-demo:/tmp`

This copies your local cuDNN download to the `/tmp` directory on your VM instance. It'll take a few seconds and you should see the percentage increase to 100% as the transfer proceeds.

## Install cuDNN on your VM
*On your VM instance* unzip/untar the cuDNN download:
...
Note 1: if the environment variable `$CUDA_HOME` isn't recognized you probably forgot to run `$ source ~/.bashrc`, so do that. If it still doesn't work, check that it was indeed added to `~/.bashrc`

Note 2: installing CUDA libraries is actually quite easy—all we did was copy the header (cudnn.h) and library files (libcudnn*) into the CUDA directory, no building or futzing around required.

# Take a break

Whew. Still hanging in there? it's okay, we're almost done.
...
`$ git clone https://github.com/rkp8000/gcp_tf_gpu_installation.git`
...
This will create a new virtual environment named "tf_demo" and install a bunch of libraries into it (including tensorflow), which will take a few minutes.
...
From now on, this is something you'll need to do every time you start a new SSH session, since it tells the system to use the Python environment where you installed all your goodies.
...
You may get a couple warnings, but if you get the output `b'Hello, TensorFlow!'` then TensorFlow has at least installed reasonably correctly.

If you get an error or the import hangs, close your SSH connection, establish a new one, activate your environment again, and trying again. If that doesn't fix it, check out [https://www.tensorflow.org/install/install_linux#common_installation_problems](https://www.tensorflow.org/install/install_linux#common_installation_problems) for common tensorflow installation problems.
...
The script will take a minute to download and format the MNIST handwritten digits dataset, then training will begin. If the GPU is being used t should only take a minute or two to run through all 10 epochs once training begins, and you should see an output like the following:
...
Phew. It works. I hope?

# Create a disk image

To not have to go through that again every time you spin up a new VM instance, it will be of great benefit to create an image of the disk you so carefully set up.
